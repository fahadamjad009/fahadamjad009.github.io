// Portfolio V3 — Black/Mint aesthetic
// Updated: NYC Taxi + Oil Supply Chain added; jamid/idclient/social/ai-job/hands-on excluded

import { useState, useEffect, useRef } from "react";

const PROJECTS = [
  { id:"nyc",     code:"// 01", name:"NYC Taxi & Rideshare Platform",  cat:"Data Engineering · ML",   desc:"Metadata-driven Azure ADF pipeline ingesting all 4 TLC vehicle classes (1.95B rows) into Databricks Delta Lake. DuckDB aggregation layer, XGBoost fare predictor (R²=0.93, RMSE=$3.70), SHAP explainability, and Plotly dark-mode dashboard. Permanent Streamlit Community Cloud deployment.",    stack:["Azure ADF","Databricks","Delta Lake","DuckDB","XGBoost","SHAP","Streamlit","Plotly"],   status:"Deployed",    sc:"#64FFDA", link:"https://github.com/fahadamjad009/nyc-taxi-azure-platform",              r:true,  e:true  },
  { id:"oil",     code:"// 02", name:"Global Oil Supply Chain",         cat:"Data Engineering · BI",   desc:"End-to-end Snowflake pipeline: EIA Open Data API ingestion (9 series, ~98K rows) → dbt staging + marts (dim_date + 9 fact tables) → interactive Streamlit dashboard covering U.S. supply balance (1920–2026), Cushing OK storage, global production, PADD-to-PADD trade flows.",               stack:["Snowflake","dbt","Python","Streamlit","EIA API"],                                        status:"Deployed",    sc:"#64FFDA", link:"https://github.com/fahadamjad009/snowflake-oil-supply-chain",           r:true,  e:true  },
  { id:"ml2",     code:"// 03", name:"LLM Regulatory Agent",            cat:"AI Engineering",           desc:"LangGraph agent for regulatory Q&A and SAR narrative generation. Hybrid FAISS+BM25 retrieval with RRF fusion, local llama3.2:3b via Ollama, FastAPI service. 40/40 tests passing.",                                                                                                            stack:["Python","LangGraph","Ollama","FAISS","BM25","FastAPI","Streamlit"],                      status:"Complete",    sc:"#64FFDA", link:"https://github.com/fahadamjad009/ml2-llm-regulatory-agent",           r:true,  e:true  },
  { id:"ml1",     code:"// 04", name:"Credit Risk Scorer",              cat:"Deep Learning",            desc:"PyTorch attention NN + XGBoost credit default risk scorer trained on 307K loan applications. AUC 0.7689, 34/34 tests, SHAP explainability, FastAPI + Streamlit. Live demo available.",                                                                                                          stack:["PyTorch","XGBoost","SHAP","FastAPI","Streamlit","Python"],                               status:"Complete",    sc:"#64FFDA", link:"https://github.com/fahadamjad009/ml1-credit-risk-scorer",             r:true,  e:true  },
  { id:"asx",     code:"// 05", name:"ASX/ABS Early Warning",           cat:"FinTech Analytics",        desc:"ML pipeline combining ASX market data with ABS macroeconomic indicators for financial risk detection. XGBoost ensemble, calibrated threshold, FastAPI scoring service, Docker CI/CD.",                                                                                                           stack:["XGBoost","Streamlit","FastAPI","Docker","Python"],                                       status:"Deployed",    sc:"#64FFDA", link:"https://github.com/fahadamjad009/asx-abs-early-warning",             r:true,  e:false },
  { id:"nlp1",    code:"// 06", name:"Financial Sentiment NLP",         cat:"NLP · Transformers",       desc:"Fine-tuned DistilBERT for 3-class financial news sentiment classification achieving 87.9% accuracy. NER pipeline for company and ticker extraction. HuggingFace + FastAPI inference.",                                                                                                         stack:["PyTorch","HuggingFace","DistilBERT","NER","FastAPI"],                                    status:"Complete",    sc:"#64FFDA", link:"https://github.com/fahadamjad009/nlp1-financial-sentiment",             r:true,  e:true  },
  { id:"de1",     code:"// 07", name:"Airflow + dbt + DuckDB",          cat:"Data Engineering",         desc:"End-to-end ASX financial data pipeline through Bronze to Silver to Gold medallion layers. Apache Airflow orchestration, dbt transformations, DuckDB analytics engine, Docker stack.",                                                                                                          stack:["Airflow","dbt","DuckDB","Docker","Python","Streamlit"],                                  status:"Complete",    sc:"#64FFDA", link:"https://github.com/fahadamjad009/de1-airflow-dbt-duckdb-pipeline",      r:true,  e:true  },
  { id:"t1",      code:"// 08", name:"RegTech RAG Engine",              cat:"RAG · NLP",                desc:"Evaluation-first RAG system with hybrid BM25 and dense retrieval with reciprocal rank fusion for Australian regulatory document Q&A. Recall@K, MRR, nDCG metrics. FastAPI service.",                                                                                                         stack:["FAISS","BM25","sentence-transformers","FastAPI","Python"],                               status:"Complete",    sc:"#64FFDA", link:"https://github.com/fahadamjad009/t1-rag-reg",                          r:true,  e:true  },
  { id:"nlp2",    code:"// 09", name:"ESG Regulatory Intelligence",     cat:"NLP · ESG",                desc:"Multi-task NLP pipeline for ESG entity extraction, regulatory topic classification, and semantic similarity across sustainability disclosures. BERTopic + custom NER.",                                                                                                                        stack:["PyTorch","HuggingFace","BERTopic","NLP","Streamlit"],                                    status:"Complete",    sc:"#64FFDA", link:"https://github.com/fahadamjad009/nlp2-esg-regulatory-intelligence",    r:true,  e:true  },
  { id:"nlp3",    code:"// 10", name:"Crypto News Intelligence",        cat:"NLP · Crypto",             desc:"Multi-label classification pipeline tagging crypto news with sentiment, asset tickers, and market impact signals. Real-time ingestion with CoinGecko entity linking. Streamlit dashboard.",                                                                                                   stack:["PyTorch","HuggingFace","Multi-label","Streamlit","Python"],                              status:"Complete",    sc:"#64FFDA", link:"https://github.com/fahadamjad009/nlp3-crypto-financial-news-intelligence", r:true,  e:true  },
  { id:"bi",      code:"// 11", name:"Financial KPI Command Centre",    cat:"BI · Analytics",           desc:"Executive-grade financial intelligence dashboard analysing US large-cap bank quarterly performance. Power BI + Python ETL from SEC EDGAR. DAX measures, waterfall charts, variance analysis.",                                                                                                  stack:["Power BI","Python","DAX","SQL","Financial Analytics"],                                   status:"Complete",    sc:"#64FFDA", link:"https://github.com/fahadamjad009/bi-financial-kpi-command-centre",      r:true,  e:false },
  { id:"tableau", code:"// 12", name:"ASX Tableau Intelligence",        cat:"BI · Visualisation",       desc:"Nine-sheet Tableau dashboard and published story on ASX sector performance across all 11 GICS sectors. Correlation matrices, rolling returns, earnings seasonality. Live on Tableau Public.",                                                                                                 stack:["Tableau","Python","SQL","yfinance"],                                                     status:"Deployed",    sc:"#64FFDA", link:"https://github.com/fahadamjad009/asx-tableau-intelligence",          r:true,  e:false },
...
function ProjectCard({ project, index }) {
  ...
  return (
    <div ref={ref} onMouseEnter={() => setHov(true)} onMouseLeave={() => setHov(false)} style={{
      ...
      <a href={project.link} target="_blank" rel="noreferrer" style={{ fontFamily:"'JetBrains Mono',monospace", fontSize:".68rem", color: hov ? "#64FFDA" : "#2a2a2a", textDecoration:"none", letterSpacing:".04em", transition:"color .2s", display:"flex", alignItems:"center", gap:5, marginTop:2 }}>
        ↗ view on github
      </a>
    </div>
  );
    ...
    <div style={{ fontFamily:"'Inter',-apple-system,sans-serif", background:"#0a0a0a", color:"#e6e6e6", minHeight:"100vh", overflowX:"hidden" }}>
      ...
      <section ref={statsRef} style={{ borderTop:"1px solid #111", borderBottom:"1px solid #111", padding:"0 clamp(24px,5vw,64px)" }}>
        <div style={{ maxWidth:1060, margin:"0 auto", display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(160px,1fr))" }}>
          {[{value:15,label:"Portfolio Systems"},{value:2,label:"Graduate Degrees"},{value:22,label:"GitHub Repos",suffix:"+"},{value:5,label:"Deployed Apps"}].map((s,i)=>(
      ...
      <section id="projects" style={{ padding:"96px clamp(24px,5vw,64px)" }}>
        <div style={{ maxWidth:1060, margin:"0 auto" }}>
          <Reveal>
            ...
            <h2 style={{ fontSize:"clamp(1.8rem,3.5vw,2.6rem)", fontWeight:800, letterSpacing:"-.025em", color:"#fff", margin:"0 0 12px", lineHeight:1.15 }}>End-to-end<br/><span style={{ color:"#222" }}>systems.</span></h2>
