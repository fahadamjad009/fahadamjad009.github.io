...
const PROJECTS = [
  ...
  { id:"ml1",     code:"// 02", name:"Credit Risk Scorer",           cat:"Deep Learning",          desc:"PyTorch attention NN + XGBoost credit default risk scorer trained on 307K loan applications. AUC 0.7689, 34/34 tests, SHAP explainability, FastAPI + Streamlit. Live demo available.",                                                stack:["PyTorch","XGBoost","SHAP","FastAPI","Streamlit","Python"],              status:"Complete",    sc:"#64FFDA", link:"https://github.com/fahadamjad009/ml1-credit-risk-scorer",             r:true,  e:true  },
  { id:"asx",     code:"// 03", name:"ASX/ABS Early Warning",        cat:"FinTech Analytics",      desc:"ML pipeline combining ASX market data with ABS macroeconomic indicators for financial risk detection. XGBoost ensemble, calibrated threshold, FastAPI scoring service, Docker CI/CD.",                                                 stack:["XGBoost","Streamlit","FastAPI","Docker","Python"],                      status:"Deployed",    sc:"#64FFDA", link:"https://github.com/fahadamjad009/asx-abs-early-warning",             r:true,  e:false },
  { id:"nlp1",    code:"// 04", name:"Financial Sentiment NLP",      cat:"NLP Â· Transformers",     desc:"Fine-tuned DistilBERT for 3-class financial news sentiment classification achieving 87.9% accuracy. NER pipeline for company and ticker extraction. HuggingFace + FastAPI inference.",                                               stack:["PyTorch","HuggingFace","DistilBERT","NER","FastAPI"],                   status:"Complete",    sc:"#64FFDA", link:"https://github.com/fahadamjad009/nlp1-financial-sentiment",             r:true,  e:true  },
  { id:"de1",     code:"// 05", name:"Airflow + dbt + DuckDB",       cat:"Data Engineering",       desc:"End-to-end ASX financial data pipeline through Bronzeâ†’Silverâ†’Gold medallion layers. Apache Airflow orchestration, dbt transformations, DuckDB analytics engine, Docker stack.",                                                      stack:["Airflow","dbt","DuckDB","Docker","Python","Streamlit"],                 status:"Complete",    sc:"#64FFDA", link:"https://github.com/fahadamjad009/de1-airflow-dbt-duckdb-pipeline",      r:true,  e:true  },
  { id:"t1",      code:"// 06", name:"RegTech RAG Engine",           cat:"RAG Â· NLP",              desc:"Evaluation-first RAG system with hybrid BM25+dense retrieval and reciprocal rank fusion for Australian regulatory document Q&A. Recall@K, MRR, nDCG metrics. FastAPI service.",                                                     stack:["FAISS","BM25","sentence-transformers","FastAPI","Python"],              status:"Complete",    sc:"#64FFDA", link:"https://github.com/fahadamjad009/t1-rag-reg",                          r:true,  e:true  },
  { id:"nlp2",    code:"// 07", name:"ESG Regulatory Intelligence",  cat:"NLP Â· ESG",              desc:"Multi-task NLP pipeline for ESG entity extraction, regulatory topic classification, and semantic similarity across sustainability disclosures. BERTopic + custom NER.",                                                                stack:["PyTorch","HuggingFace","BERTopic","NLP","Streamlit"],                   status:"Complete",    sc:"#64FFDA", link:"https://github.com/fahadamjad009/nlp2-esg-regulatory-intelligence",    r:true,  e:true  },
  { id:"nlp3",    code:"// 08", name:"Crypto News Intelligence",     cat:"NLP Â· Crypto",           desc:"Multi-label classification pipeline tagging crypto news with sentiment, asset tickers, and market impact signals. Real-time ingestion with CoinGecko entity linking. Streamlit dashboard.",                                          stack:["PyTorch","HuggingFace","Multi-label","Streamlit","Python"],             status:"Complete",    sc:"#64FFDA", link:"https://github.com/fahadamjad009/nlp3-crypto-financial-news-intelligence", r:true,  e:true  },
  { id:"bi",      code:"// 09", name:"Financial KPI Command Centre", cat:"BI Â· Analytics",         desc:"Executive-grade financial intelligence dashboard analysing US large-cap bank quarterly performance. Power BI + Python ETL from SEC EDGAR. DAX measures, waterfall charts, variance analysis.",                                        stack:["Power BI","Python","DAX","SQL","Financial Analytics"],                  status:"Complete",    sc:"#64FFDA", link:"https://github.com/fahadamjad009/bi-financial-kpi-command-centre",      r:true,  e:false },
  { id:"tableau", code:"// 10", name:"ASX Tableau Intelligence",     cat:"BI Â· Visualisation",     desc:"Nine-sheet Tableau dashboard and published story on ASX sector performance across all 11 GICS sectors. Correlation matrices, rolling returns, earnings seasonality. Live on Tableau Public.",                                        stack:["Tableau","Python","SQL","yfinance"],                                    status:"Deployed",    sc:"#64FFDA", link:"https://github.com/fahadamjad009/asx-tableau-intelligence",          r:true,  e:false },
  { id:"fraud",   code:"// 11", name:"FinTech Fraud Detection",      cat:"FinTech Â· ML",           desc:"End-to-end fraud detection platform with XGBoost champion model. Cost-sensitive threshold optimisation, Platt scaling calibration, velocity features, merchant risk scores. FastAPI + Streamlit.",                                   stack:["XGBoost","FastAPI","Streamlit","SHAP","scikit-learn"],                  status:"Complete",    sc:"#64FFDA", link:"https://github.com/fahadamjad009/fintech-fraud-detection-platform",      r:true,  e:true  },
  { id:"churn",   code:"// 12", name:"Churn ML Benchmark",           cat:"Machine Learning",       desc:"Reproducible benchmarking pipeline comparing XGBoost, LightGBM, and Logistic Regression for customer churn. CLV-weighted business metrics, ROC/AUC comparison, SHAP drivers, campaign ROI simulator.",                             stack:["XGBoost","LightGBM","SHAP","Streamlit","scikit-learn"],                status:"Complete",    sc:"#64FFDA", link:"https://github.com/fahadamjad009/customer-churn-ml-benchmark",          r:false, e:true  },
  { id:"mining",  code:"// 13", name:"Mining Operations Analytics",  cat:"Industrial Analytics",   desc:"Predictive maintenance platform for mining equipment using telemetry data. SHAP explainability, KS drift detection, OEE benchmarking, NASA C-MAPSS benchmark. Deployed Streamlit demo.",                                            stack:["Python","scikit-learn","SHAP","Streamlit","Predictive Analytics"],      status:"Complete",    sc:"#64FFDA", link:"https://github.com/fahadamjad009/mining-operations-analytics-platform",  r:false, e:true  },
  { id:"jamid",   code:"// 14", name:"JAMID Identity Wallet",        cat:"Decentralised Identity", desc:"Open-source Android identity wallet implementing OID4VCI/OID4VP credential flows with Keycloak-backed issuer portal, verifier gateway, and policy engine. W3C Verifiable Credentials.",                                             stack:["Android","Keycloak","OID4VCI","Next.js","PostgreSQL"],                  status:"In Progress", sc:"#F59E0B", link:"https://github.com/fahadamjad009/Jamid",                              r:true,  e:true  },
  { id:"idclient",code:"// 15", name:"idclient Platform",            cat:"Identity Â· TypeScript",  desc:"TypeScript identity client platform for decentralised credential management and verification flows.",                                                                                                                                  stack:["TypeScript","Next.js","PostgreSQL"],                                    status:"In Progress", sc:"#F59E0B", link:"https://github.com/fahadamjad009/idclient-platform",                r:false, e:true  },
];
    ...
    <div style={{ fontFamily:"'Inter',-apple-system,sans-serif", background:"#0a0a0a", color:"#e6e6e6", minHeight:"100vh", overflowX:"hidden" }}>
      ...
      <section ref={statsRef} style={{ borderTop:"1px solid #111", borderBottom:"1px solid #111", padding:"0 clamp(24px,5vw,64px)" }}>
        <div style={{ maxWidth:1060, margin:"0 auto", display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(160px,1fr))" }}>
          {[{value:15,label:"Portfolio Systems"},{value:2,label:"Graduate Degrees"},{value:20,label:"GitHub Repos",suffix:"+"},{value:3,label:"Deployed Apps"}].map((s,i)=>(
      ...
      <section id="projects" style={{ padding:"96px clamp(24px,5vw,64px)" }}>
        <div style={{ maxWidth:1060, margin:"0 auto" }}>
          <Reveal>
            ...
            <h2 style={{ fontSize:"clamp(1.8rem,3.5vw,2.6rem)", fontWeight:800, letterSpacing:"-.025em", color:"#fff", margin:"0 0 12px", lineHeight:1.15 }}>Fifteen end-to-end<br/><span style={{ color:"#222" }}>systems.</span></h2>
